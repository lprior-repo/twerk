...
use super::encrypt;
use twerk_core::{
    id::{JobId, NodeId, ScheduledJobId, TaskId},
    job::{Job, JobContext, JobDefaults, JobSchedule, JobState, ScheduledJob, ScheduledJobState},
    node::{Node, NodeStatus},
    role::Role,
    task::{AutoDelete, Permission, Task, TaskLogPart, TaskState},
    user::User,
    webhook::Webhook,
};
...
impl TaskRecord {
    ...
    pub fn to_task(&self) -> Result<Task, DatastoreError> {
        ...
        Ok(Task {
            ...
            node_id: self.node_id.as_ref().map(|id| NodeId::new(id.clone())),
            retry,
            limits,
...
impl JobRecord {
    ...
    pub fn to_job(
        ...
        let webhooks: Vec<Webhook> = self
            ...
            .as_ref()
            .and_then(|bytes| serde_json::from_slice(bytes).ok())
            .map_or_else(Vec::new, |v| v);

        let mut secrets: HashMap<String, String> = self
            .secrets
            .as_ref()
            .and_then(|bytes| serde_json::from_slice(bytes).ok())
            .map_or_else(HashMap::new, |v| v);
