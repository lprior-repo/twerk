...
#[tokio::test]
async fn engine_shutdown_cancels_pending_tasks() -> Result<()> {
    std::env::set_var("TWERK_DATASTORE_TYPE", "inmemory");
    std::env::set_var("TWERK_BROKER_TYPE", "inmemory");
    ...
    for i in 0..5 {
        let task = Task {
            name: Some(format!("pending task {}", i)),
            image: Some("alpine".to_string()),
            run: Some("sleep 10".to_string()),
    ...
    assert!(
        queue_info.size >= 5,
        "tasks were submitted to queue and remain there after shutdown"
    );
...
#[tokio::test]
async fn engine_shutdown_fails_when_not_running() {
    let mut engine = engine_with_mode(Mode::Standalone);
...
#[tokio::test]
async fn engine_shutdown_terminates_running_engine() -> Result<()> {
    std::env::set_var("TWERK_DATASTORE_TYPE", "inmemory");
    std::env::set_var("TWERK_BROKER_TYPE", "inmemory");
...
#[tokio::test]
async fn engine_shutdown_rejects_new_tasks_after_shutdown() -> Result<()> {
    std::env::set_var("TWERK_DATASTORE_TYPE", "inmemory");
    std::env::set_var("TWERK_BROKER_TYPE", "inmemory");
    ...
    let task = Task {
        name: Some("task after shutdown".to_string()),
        image: Some("alpine".to_string()),
        run: Some("echo hello".to_string()),
        ..Default::default()
    };
    let result = engine.submit_task(task).await;
    assert!(
        matches!(result, Err(twerk_app::engine::SubmitTaskError::NotRunning)),
        "submitting task after shutdown should return NotRunning error"
    );
