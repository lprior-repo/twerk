...
/// Invokes a handler directly (synchronous call in async context).
/// Use for small numbers of handlers where spawn overhead dominates.
#[inline]
fn invoke_handler_direct<T: Clone>(
    handler: &Arc<dyn Fn(T) -> BoxedFuture<()> + Send + Sync>,
    msg: T,
    _error_msg: &'static str,
) {
    // Use now_or_never to check if the future is ready (should be for fast handlers)
    if let Some(Err(e)) = handler(msg).now_or_never() {
