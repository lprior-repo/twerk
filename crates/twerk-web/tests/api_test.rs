    ...
    let response = app
        .oneshot(
            axum::http::Request::builder()
                ...
                .body(axum::body::Body::from(serde_json::to_vec(&job_input).unwrap()))
                .unwrap(),
        )
        .await
        .unwrap();
...
async fn job_wait_returns_completed_when_job_finishes() {
    ...
    impl twerk_infrastructure::broker::Broker for SignalingBroker {
        ...
        fn subscribe_for_events(&self, pattern: String, handler: twerk_infrastructure::broker::EventHandler) -> twerk_infrastructure::broker::BoxedFuture<()> {
        ...
        fn health_check(&self) -> twerk_infrastructure::broker::BoxedFuture<()> { self.inner.health_check() }
        fn shutdown(&self) -> twerk_infrastructure::broker::BoxedFuture<()> { self.inner.shutdown() }
    }

    let broker = Arc::new(SignalingBroker { inner: InMemoryBroker::new(), tx: sub_tx });
    ...
    tokio::spawn(async move {
        ...
        broker_clone.publish_event("job.completed".to_string(), finished_job).await.unwrap();
    });
