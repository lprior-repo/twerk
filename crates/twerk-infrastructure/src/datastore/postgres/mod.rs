...
impl Datastore for PostgresDatastore {
    async fn create_job(&self, job: &Job) -> DatastoreResult<()> {
        ...
        let query = sqlx::query(q)
            ...
            .bind(
                job.schedule
                    .as_ref()
                    .and_then(|s| s.id.as_ref().map(|id| id.to_string())),
            )
            .bind(job.started_at)
    ...
    async fn create_task(&self, task: &Task) -> DatastoreResult<()> {
        ...
        let query = sqlx::query(q)
            ...
            .bind(&sidecars)
            .bind(&mounts)
            .bind(task.node_id.as_ref().map(|id| id.as_str()))
            .bind(&retry)
            .bind(&limits)
            .bind(&task.timeout)
            .bind(&task.result)
            .bind(&task.var)
            .bind(&parallel)
            .bind(task.parent_id.as_ref().map(|id| id.as_str()))
            .bind(&each)
                ...
                let parallel: Option<Vec<u8>> = task
                    ...
                    .map(|v| {
                        serde_json::to_vec(v).map_err(|e| {
                            DatastoreError::Serialization(format!("task.parallel: {e}"))
                        })
                    })
                    .transpose()?;
                let each: Option<Vec<u8>> = task
    ...
    async fn create_node(&self, node: &Node) -> DatastoreResult<()> {
        ...
        let query = sqlx::query(q)
            ...
            .bind(node.cpu_percent.map_or(0.0, |p| p))
            .bind(status.as_ref())
            .bind(hostname)
            .bind(node.port.map_or(0, |p| p))
            .bind(node.task_count.map_or(0, |p| p))
            .bind(&node.version);
        match &self.executor {
            Executor::Pool(p) => {
                query
    ...
    async fn get_job_log_parts(
        ...
        Ok(Page {
            ...
            number: page,
            size,
            total_pages: (count as f64 / size as f64).ceil() as i64,
            total_items: count,
        })
    }
