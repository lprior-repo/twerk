...
pub use engine::{
    Config, EndpointHandler, Engine, JobEventType, JobHandlerFunc, JobListener,
    JobMiddlewareFunc, LogHandlerFunc, LogMiddlewareFunc, Middleware, Mode, NodeHandlerFunc,
    NodeMiddlewareFunc, State, TaskEventType, TaskHandlerFunc, TaskMiddlewareFunc,
    WebMiddlewareFunc,
};
...
mod engine {
    ...
    use super::worker::{create_worker, Worker};
    ...
    pub type JobListener = Arc<dyn Fn(twerk_core::job::Job) + Send + Sync>;
    ...
    pub type TaskHandlerFunc = Arc<
        dyn Fn(Arc<()>, TaskEventType, &mut twerk_core::task::Task) -> Result<(), TaskHandlerError>
            + Send
            + Sync,
    >;
    ...
    pub type JobHandlerFunc = Arc<
        dyn Fn(Arc<()>, JobEventType, &mut twerk_core::job::Job) -> Result<(), JobHandlerError>
            + Send
            + Sync,
    >;
    ...
    pub type LogHandlerFunc = Arc<
        dyn Fn(Arc<()>, &[twerk_core::task::TaskLogPart]) -> Result<(), LogHandlerError> + Send + Sync,
    >;
                    ...
                    .subscribe_for_events(
                        ...
                        Arc::new(move |event: serde_json::Value| {
                            ...
                            Box::pin(async move {
                                ...
                                if let Ok(ev_job) = serde_json::from_value::<twerk_core::job::Job>(event)
                                {
    ...
    fn resolve_locker_type() -> String {
        let from_env = std::env::var("TWERK_LOCKER_TYPE")
            .ok()
            .filter(|s| !s.is_empty());
        match from_env {
            Some(t) => t,
            None => std::env::var("TWERK_DATASTORE_TYPE")
                .ok()
                .filter(|s| !s.is_empty())
    ...
    impl Runtime for MockRuntime {
        fn run(&self, _task: &twerk_core::task::Task) -> twerk_infrastructure::runtime::BoxedFuture<()> {
            Box::pin(async { Ok(()) })
        }

        fn stop(&self, _task: &twerk_core::task::Task) -> twerk_infrastructure::runtime::BoxedFuture<()> {
            Box::pin(async { Ok(()) })
        }
    ...
    mod tests {
        ...
        /// Helper: set `TWERK_DATASTORE_TYPE=inmemory` for tests that need a datastore.
        fn ensure_inmemory_datastore_env() {
            std::env::set_var("TWERK_DATASTORE_TYPE", "inmemory");
        }

        /// Helper: reset `TWERK_DATASTORE_TYPE` to inmemory after test.
        ...
        fn test_register_mounter_creates_multi_mounter() {
            ...
            impl Mounter for NoopMounter {
                fn mount(&self, _mnt: &Mount) -> twerk_infrastructure::runtime::BoxedFuture<()> {
                    Box::pin(async { Ok(()) })
                }

                fn unmount(&self, _mnt: &Mount) -> twerk_infrastructure::runtime::BoxedFuture<()> {
                    Box::pin(async { Ok(()) })
                }
            }
        ...
        impl Mounter for FakeTestMounter {
            fn mount(&self, _mnt: &twerk_core::mount::Mount) -> twerk_infrastructure::runtime::BoxedFuture<()> {
                Box::pin(async { Ok(()) })
            }

            fn unmount(&self, _mnt: &twerk_core::mount::Mount) -> twerk_infrastructure::runtime::BoxedFuture<()> {
                Box::pin(async { Ok(()) })
            }
        }
        ...
        async fn test_submit_job_when_not_running() {
            let engine = Engine::new(Config::default());
            let job = twerk_core::job::Job::default();
        ...
        async fn test_register_datastore_provider() {
            let mut engine = Engine::new(Config::default());
            engine
                .register_datastore_provider("custom", crate::engine::datastore::new_inmemory_datastore());
            assert!(
        ...
        async fn test_register_datastore_provider_duplicate_ignored() {
            let mut engine = Engine::new(Config::default());
            engine.register_datastore_provider("pg", crate::engine::datastore::new_inmemory_datastore());
            engine.register_datastore_provider("pg", crate::engine::datastore::new_inmemory_datastore());
        ...
        async fn test_register_broker_provider() {
            let mut engine = Engine::new(Config::default());
            engine
                .register_broker_provider("custom", Box::new(twerk_infrastructure::broker::inmemory::new_in_memory_broker()));
            assert!(
        ...
        async fn test_register_broker_provider_duplicate_ignored() {
            let mut engine = Engine::new(Config::default());
            engine.register_broker_provider("rmq", Box::new(twerk_infrastructure::broker::inmemory::new_in_memory_broker()));
            engine.register_broker_provider("rmq", Box::new(twerk_infrastructure::broker::inmemory::new_in_memory_broker()));
