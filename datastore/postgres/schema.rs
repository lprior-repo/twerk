...
mod tests {
    ...
    #[test]
    fn schema_contains_index_jobs_ts() {
        assert!(
            SCHEMA.contains("CREATE INDEX jobs_ts_idx ON jobs USING GIN"),
            "SCHEMA must have GIN index on jobs.ts",
        );
    }
    ...
    #[test]
    fn schema_contains_index_tasks_log_parts_ts() {
        assert!(
            SCHEMA.contains("CREATE INDEX tasks_log_parts_ts_idx ON tasks_log_parts USING GIN"),
            "SCHEMA must have GIN index on tasks_log_parts.ts",
        );
    }
    ...
    #[test]
    fn schema_jobs_has_tsvector_column() {
        assert!(
            SCHEMA.contains("ADD COLUMN ts tsvector NOT NULL"),
            "jobs must have a tsvector column 'ts'",
        );
    }

    #[test]
    fn schema_tasks_log_parts_has_tsvector_column() {
        assert!(
            SCHEMA.contains("tasks_log_parts ADD COLUMN ts tsvector NOT NULL"),
            "tasks_log_parts must have a tsvector column 'ts'",
        );
    }
    ...
    #[test]
    fn schema_jobs_progress_is_numeric() {
        assert!(
            normalized_schema().contains("progress numeric(5,2) default 0"),
            "jobs.progress must be numeric(5,2)",
        );
    }

    #[test]
    fn schema_tasks_progress_is_numeric() {
        // Count: one in jobs, one in tasks
        let count = normalized_schema()
            .matches("progress numeric(5,2) default 0")
            .count();
        assert_eq!(
            count, 2,
            "both jobs and tasks must have progress numeric(5,2)"
        );
    }
    ...
    fn schema_scheduled_jobs_tags_is_text_array() {
        // Count occurrences of text array tags
        let count = normalized_schema()
