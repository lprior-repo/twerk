...
pub async fn register_mounter(
    rt: &str,
    name: &str,
    mounter: Box<dyn twerk_infrastructure::runtime::Mounter>,
) {
    let mut engine = DEFAULT_ENGINE.write().await;
...
///
/// Go parity: `func SubmitJob(ctx context.Context, ij *input.Job, listeners ...JobListener) (*twerk.Job, error)`
pub async fn submit_job(
    job: twerk_core::job::Job,
    listeners: Vec<JobListener>,
) -> anyhow::Result<twerk_core::job::Job> {
...
pub async fn broker() -> crate::engine::BrokerProxy {
    let engine = DEFAULT_ENGINE.read().await;
    engine.broker_proxy()
}
...
pub async fn datastore() -> crate::engine::DatastoreProxy {
    let engine = DEFAULT_ENGINE.read().await;
