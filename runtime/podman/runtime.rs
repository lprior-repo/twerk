...
pub const DEFAULT_WORKDIR: &str = "/tork/workdir";
...
impl PodmanRuntime {
    ...
    async fn do_run_inner(&self, task: &mut Task) -> Result<(), PodmanError> {
        ...
        let mut create_cmd = Command::new("podman");
        create_cmd.arg("create").arg("-v").arg(format!("{}:/tork", workdir.display())).arg("--entrypoint").arg(&entrypoint[0]);
        let env_vars: Vec<String> = task.env.iter().map(|(k, v)| format!("{}={}", k, v)).chain(["TORK_OUTPUT=/tork/output".to_string(), "TORK_PROGRESS=/tork/progress".to_string()]).collect();
