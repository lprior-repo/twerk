...
impl Datastore for FailableDatastore {
    async fn create_task(&self, task: &Task) -> DatastoreResult<()> {
        if self.config.read().unwrap().fail_create_task {
            return Err(DatastoreError::Database(
                "simulated create_task failure".into(),
            ));
        }
    ...
    async fn update_node(
        ...
        id: &str,
        modify: Box<
            dyn FnOnce(twerk_core::node::Node) -> DatastoreResult<twerk_core::node::Node> + Send,
        >,
    ) -> DatastoreResult<()> {
    ...
    async fn create_job(&self, job: &Job) -> DatastoreResult<()> {
        if self.config.read().unwrap().fail_create_job {
            return Err(DatastoreError::Database(
                "simulated create_job failure".into(),
            ));
        }
    ...
    async fn update_job(
        ...
        if self.config.read().unwrap().fail_update_job {
            return Err(DatastoreError::Database(
                "simulated update_job failure".into(),
            ));
        }
    ...
    async fn get_scheduled_jobs(
        ...
        page: i64,
        size: i64,
    ) -> DatastoreResult<Page<twerk_core::job::ScheduledJobSummary>> {
        self.inner
...
async fn submit_job_returns_error_when_datastore_create_fails() -> Result<()> {
    ...
    broker
        .set_broker(Box::new(FailableBroker {
    ...
    datastore
        .set_datastore(Box::new(FailableDatastore {
        ...
        loop {
                ...
                .await
            {
                Ok(persisted_job) if persisted_job.state != JobState::Pending => {
                    ...
                    let _ = tx.send(Ok(persisted_job)).ok();
                    return;
                }
                Err(e) => {
