...
impl ConfigState {
    ...
    fn strings_for_key(&self, key: &str) -> Vec<String> {
        self.get_array(key)
            .map_or_else(Vec::new, |arr: &toml::value::Array| {
                arr.iter()
...
mod tests {
    ...
    fn cleanup() {
        ...
        // Clear TWERK_* env vars that tests set - essential for isolation
        for (k, _) in env::vars() {
            if k.starts_with("TWERK_") {
                env::remove_var(&k);
            }
        }
    }
    ...
    fn setup() {
        ...
        *guard = false; // Mark as taken
        ...
        // Clear TWERK_* env vars first - ensures isolated state from shell or previous tests
        for (k, _) in env::vars() {
            if k.starts_with("TWERK_") {
                env::remove_var(&k);
            }
    ...
    fn test_load_config_not_exist_user_defined() {
        ...
        env::set_var("TWERK_CONFIG", path.to_string_lossy().as_ref());
        let result = load_config();
        cleanup();
...
"#,
    ...
    fn test_strings_env() {
        setup();
        env::set_var("TWERK_MAIN_STRINGS_KEYS", "a,b,c");
        load_config().ok();
...
"#,
        ...
        env::set_var("TWERK_CONFIG", path.to_string_lossy().as_ref());
        load_config().ok();
        let result = duration_default("main.some.duration", time::Duration::minutes(60));
        cleanup();
        assert_eq!(result, time::Duration::minutes(90));
    }

    #[test]
    fn test_bool_map() {
        setup();
        env::set_var("TWERK_BOOLMAP_KEY1", "false");
        env::set_var("TWERK_BOOLMAP_KEY2", "true");
        load_config().ok();
...
"#,
        ...
        env::set_var("TWERK_CONFIG", path.to_string_lossy().as_ref());
        env::set_var("TWERK_MAIN_KEY1", "value2");
        load_config().ok();
...
"#,
        ...
        env::set_var("TWERK_CONFIG", path.to_string_lossy().as_ref());
        load_config().ok();
        assert_eq!("/var/tmp/twerk", mounts_temp_dir());
        cleanup();
    }
...
"#,
    ...
    fn test_bool_env_override() {
        ...
        load_config().ok();
        let result = bool("main.enabled");
        cleanup();
        assert!(result);
    }

    #[test]
    fn test_int_env_override() {
        setup();
        env::set_var("TWERK_MAIN_COUNT", "42");
        load_config().ok();
        let result = int("main.count");
        cleanup();
        assert_eq!(result, 42);
    }
