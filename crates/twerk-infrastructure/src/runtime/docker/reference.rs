...
mod tests {
    ...
    /// Strategy for generating syntactically valid Docker reference strings.
    /// Covers: simple names, names with tags, names with domain/port, and digests.
    fn valid_reference_strategy() -> impl Strategy<Value = String> {
        ...
        let path = prop_oneof![
            "[a-z][a-z0-9._-]{0,15}".prop_map(String::from),
            "[a-z][a-z0-9._-]{0,8}".prop_map(|c: String| format!("{c}/img")),
        ];
        let tag = prop_oneof![
            Just(String::new()),
            "[a-zA-Z0-9][a-zA-Z0-9._-]{0,15}".prop_map(|t| format!(":{t}")),
        ];
        (domain, path, tag).prop_map(|(dom, p, t): (String, String, String)| {
            if dom.is_empty() {
                format!("{p}{t}")
            } else {
                format!("{dom}/{p}{t}")
    ...
    proptest! {
        /// If parse succeeds, full_name() is non-empty and at most 255 characters.
        #[test]
        fn parse_then_full_name_nonempty(input in valid_reference_strategy()) {
            if let Ok(reference) = parse(&input) {
                let name = reference.full_name();
                prop_assert!(!name.is_empty(), "full_name must be non-empty");
                prop_assert!(
                    name.len() <= 255,
                    "full_name must be at most 255 chars, got {}",
                    name.len()
                );
            }
        }

        /// Empty string is always rejected by parse.
        #[test]
        fn parse_rejects_empty(_empty in Just("")) {
            let result = parse(_empty);
            prop_assert!(result.is_err(), "empty string must not parse");
        }
    }
}
