...
use crate::docker::helpers::{parse_go_duration, parse_tar_contents};
use crate::docker::tork::Probe;
...
impl Container {
    ...
    pub async fn wait(&self) -> Result<String, DockerError> {
        ...
        if status_code != 0 { return Err(DockerError::NonZeroExit(status_code, self.read_logs_tail(10).await.unwrap_or_default())); }
    ...
    async fn stream_logs(client: Docker, container_id: String, task_id: String, broker: Option<Arc<dyn tork::broker::Broker>>) {
        ...
        while let Some(result) = stream.next().await {
            match result {
                Ok(bollard::container::LogOutput::StdOut { message }) | Ok(bollard::container::LogOutput::StdErr { message }) => {
                    ...
                    if !msg.is_empty() { part_num += 1; let _ = broker.publish_task_log_part(&tork::task::TaskLogPart { id: None, number: part_num, task_id: Some(task_id.clone()), contents: Some(msg), created_at: None }).await; }
                }
                _ => {}
            }
        }
    ...
    async fn report_progress(client: Docker, container_id: String, task_id: String, broker: Option<Arc<dyn tork::broker::Broker>>) {
        use std::time::Duration;
        let Some(broker) = broker else { return };
            ...
            tokio::select! { _ = tick.tick() => {
                match Self::read_progress_value(&client, &container_id).await {
                    Ok(p) if prev.map(|old| (old - p).abs() > 0.001).unwrap_or(true) => {
                        prev = Some(p);
                        let tork_task = tork::task::Task { id: Some(task_id.clone()), progress: p, ..Default::default() };
                        if let Err(e) = broker.publish_task_progress(&tork_task).await { tracing::warn!(task_id = %task_id, error = %e, "error publishing task progress"); }
                    }
                    Err(_) => break,
                    _ => {}
                }
    ...
    pub async fn remove(&self) {
        ...
        if let Some(ref source) = self.torkdir_source { let _ = self.client.remove_volume(source, Some(bollard::volume::RemoveVolumeOptions { force: true })).await; }
    }
}
