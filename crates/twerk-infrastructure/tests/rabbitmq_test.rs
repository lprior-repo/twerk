...
use twerk_infrastructure::broker::{queue, Broker, RabbitMQOptions};
...
async fn heartbeat_delivered_when_published_to_rabbitmq() -> anyhow::Result<()> {
    ...
    broker
        .subscribe_for_heartbeats(Arc::new(move |_node| {
            let tx = tx.clone();
            Box::pin(async move {
                tx.send(()).await.map_err(|e| anyhow::anyhow!(e))?;
                Ok(())
...
async fn job_delivered_when_published_to_rabbitmq() -> anyhow::Result<()> {
    ...
    broker
        .subscribe_for_jobs(Arc::new(move |_job| {
            let tx = tx.clone();
            Box::pin(async move {
                tx.send(()).await.map_err(|e| anyhow::anyhow!(e))?;
                Ok(())
...
async fn events_routed_correctly_when_patterns_match() -> anyhow::Result<()> {
    ...
    for _ in 0..10 {
        broker
            .publish_event("job.completed".to_string(), event_payload.clone())
            .await?;
        broker
            .publish_event("job.failed".to_string(), event_payload.clone())
            .await?;
        broker
            .publish_event("job.x.y.z".to_string(), event_payload.clone())
            .await?;
    }
    ...
    for _ in 0..30 {
        tokio::time::timeout(Duration::from_secs(5), rx1.recv())
            .await?
            .ok_or_else(|| anyhow::anyhow!("rx1 timeout"))?;
    }

    for _ in 0..10 {
        tokio::time::timeout(Duration::from_secs(5), rx2.recv())
            .await?
            .ok_or_else(|| anyhow::anyhow!("rx2 timeout"))?;
    }
...
async fn test_rabbitmq_all() -> anyhow::Result<()> {
    ...
    broker
        .subscribe_for_tasks(
            queue::QUEUE_REDELIVERIES.to_string(),
            Arc::new(move |_| {
                let tx = tx.clone();
                Box::pin(async move {
                    let _ = tx.send(()).await;
    ...
    broker
        .publish_task(queue::QUEUE_REDELIVERIES.to_string(), &task)
        .await?;
