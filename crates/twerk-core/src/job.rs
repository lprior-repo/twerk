...
impl JobContext {
    ...
    pub fn as_map(&self) -> HashMap<String, serde_json::Value> {
        ...
        if let Some(ref inputs) = self.inputs {
            m.insert(
                String::from("inputs"),
                serde_json::to_value(inputs).map_or_else(|_| serde_json::Value::Null, |v| v),
            );
        }
        if let Some(ref secrets) = self.secrets {
            m.insert(
                String::from("secrets"),
                serde_json::to_value(secrets).map_or_else(|_| serde_json::Value::Null, |v| v),
            );
        }
        if let Some(ref tasks) = self.tasks {
            m.insert(
                String::from("tasks"),
                serde_json::to_value(tasks).map_or_else(|_| serde_json::Value::Null, |v| v),
            );
        }
        if let Some(ref job) = self.job {
            m.insert(
                String::from("job"),
                serde_json::to_value(job).map_or_else(|_| serde_json::Value::Null, |v| v),
            );
        }
        m
    }
