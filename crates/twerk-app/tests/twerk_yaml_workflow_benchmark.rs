...
#[tokio::test]
#[ignore = "requires running twerk server on localhost:8000"]
async fn twerk_simple_curl_workflow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    print_result(
        "Throughput",
        format!("{:.1} tasks/sec", 1.0 / total_time.as_secs_f64()),
    );

    assert_eq!(
        final_job.state,
        JobState::Completed,
        "Simple job should complete"
    );
...
async fn twerk_pokemon_api_workflow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    print_result(
        "Avg ms/task",
        format!("{:.2}", total_time.as_millis() as f64 / total_tasks as f64),
    );
