...
pub fn redact_scheduled_job_summary(sj: &mut ScheduledJobSummary) {
    // Summary doesn't have secrets map, but inputs might contain sensitive values
    // Redact by key name only (PASSWORD, SECRET, TOKEN, etc.)
    if let Some(ref mut inputs) = sj.inputs {
        *inputs = redact_vars_by_key_only(inputs);
    }
}
