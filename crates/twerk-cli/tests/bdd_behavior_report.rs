...
//! | 5 | commands | `Commands::ServerStart { mode }` accepts standalone/coordinator/worker | commands.rs:153-159 |
//! | 6 | commands | `Commands::Health { endpoint }` accepts optional endpoint | commands.rs:160-163 |
//! | 7 | cli | `Cli --json` global flag enables JSON output mode | commands.rs:14-15 |
//! | 8 | cli | `--version` short-circuits with DisplayVersion error | commands.rs |
//! | 9 | error | `CliError` variants format with expected messages | error.rs |
//! | 10 | health | `HealthResponse` deserializes from JSON | health.rs:10-14 |
//! | 11 | migrate | `run_migration("postgres", dsn)` executes schema | migrate.rs:35-61 |
//! | 12 | migrate | `run_migration("mysql", dsn)` returns `CliError::UnknownDatastore` | migrate.rs:59 |
//! | 13 | migrate | `DEFAULT_POSTGRES_DSN` uses placeholders, not real credentials | migrate.rs |
//!
//! ## Execution Evidence
...
#[test]
fn claim_7_server_start_command_accepts_standalone_mode() {
    let args = vec!["twerk", "server-start", "standalone"];
    let result = Cli::try_parse_from(args);
    let cli = result.expect("standalone mode should be accepted");
    assert!(
        matches!(cli.command, Some(Commands::ServerStart { .. })),
        "expected ServerStart command, got {:?}",
        cli.command
    );
}

#[test]
fn claim_8_server_start_command_accepts_coordinator_mode() {
    let args = vec!["twerk", "server-start", "coordinator"];
    let cli = Cli::try_parse_from(args).expect("coordinator mode should be accepted");
    match cli.command {
        Some(Commands::ServerStart { mode, .. }) => {
            assert_eq!(mode, RunMode::Coordinator, "expected Coordinator mode");
        }
        other => panic!("expected ServerStart command, got {:?}", other),
    }
}

#[test]
fn claim_9_server_start_command_accepts_worker_mode() {
